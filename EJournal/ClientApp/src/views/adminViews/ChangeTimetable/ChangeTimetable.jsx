...
class ChangeTimetable extends React.Component {
  ...
  render() {
      const {loading,teachers,groups,subjects,failed,auditories,succes} = this.props;
    ...
    const handleChangeMultiple = (event) => {
      ...
      if(dateFrom != null && event.target.value.length!=0 &&  number.length!=0 && dateTo!=null)
      this.props.getAuditoriums({dateFrom:dateFrom,dateTo:dateTo,group:group,daysOfWeek:event.target.value,
      numbers:number})
    };
    const handleChangeMultipleNumber = (event) => {
        this.setState({ number: event.target.value });
        const {dateFrom,daysOfWeek,number,dateTo,group} = this.state;
        if(dateFrom != null && daysOfWeek.length!=0 &&  event.target.value.length!=0 && dateTo!=null)
        this.props.getAuditoriums({dateFrom:dateFrom,dateTo:dateTo,group:group,daysOfWeek:daysOfWeek,
    ...
    const handleChangeGroup = (event) => {
        ...
        if(dateFrom != null && daysOfWeek.length!=0 && number.length!=0 && dateTo!=null)
        this.props.getAuditoriums({dateFrom:dateFrom,dateTo:dateTo,group:event.target.value,daysOfWeek:daysOfWeek,
        numbers:number})
      };
      ...
      const handleDateFromChange = (date) => {
        ...
        const {dateFrom,daysOfWeek,number,dateTo,group} = this.state;
        if(date != null && daysOfWeek.length!=0 && number.length!=0 && dateTo!=null)
        this.props.getAuditoriums({dateFrom:date,dateTo:dateTo,group:group,daysOfWeek:daysOfWeek,
        numbers:number})
      };
      const handleDateToChange = (date) => {
        this.setState({dateTo:date});
        const {dateFrom,daysOfWeek,number,dateTo,group} = this.state;
        if(dateFrom != null && daysOfWeek.length!=0 && number.length!=0 && date!=null)
        this.props.getAuditoriums({dateFrom:dateFrom,dateTo:date,group:group,daysOfWeek:daysOfWeek,
        numbers:number})
      };
      const save = () => {
        const {dateFrom,daysOfWeek,number,dateTo,group,auditory,subject,teacher} = this.state;
        if(dateFrom != null && daysOfWeek.length!=0 && number.length!=0 && dateTo!=null &&
            group!="" &&  auditory!=""  &&  subject!=""  &&  teacher!="" ){
        this.props.save({dateFrom:dateFrom,dateTo:dateTo,group:group,daysOfWeek:daysOfWeek,
        numbers:number,auditory:auditory,subject:subject,teacher:teacher})
        }
        else{
            this.growl.show({
                        severity: "error",
    ...
    return (
      <React.Fragment>
        ...
        <Grid container spacing={4} className="mt-2">
            ...
            <FormControl variant="outlined" style={{ width: "100%" }}>
              ...
              >
                  {teachers.map(function(el){
                      return(
                      <MenuItem key={el.id} value={el.id}>{el.name+" "+el.surname+" "+el.lastName}</MenuItem>
                      );
                  })}
              </Select>
            </FormControl>
          ...
          </Grid><Grid item lg={3} md={4} xl={2} xs={6}>
          <RadioGroup
            style={{ width: "100%" }}            
            className="ml-2"
